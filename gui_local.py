...
class VideoPlayer(QWidget):
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Right:  # 检查是否按下右箭头
            self.next_f_button.click()
    ...
    def save_result(self):
        ...
        for video_name in self.anno.keys():
            self.anno[video_name]['click_points'] = {}
            self.anno[video_name]['click_points']['pos'] = []
            self.anno[video_name]['click_points']['raw_pos'] = []

            for frame_id, frame_anno in self.tracking_points.items():
                for k, v in frame_anno.items():
                    if len(v)!=0 and (k in ['pos', 'raw_pos']):
                        self.anno[video_name]['click_points'][k].extend([np.array([v[0].x(), v[0].y()])])
                    elif (k in ['pos', 'raw_pos']):
                        self.anno[video_name]['click_points'][k].extend([np.array([-1, -1])])

        
        cam_name = video_name.split('/')[-1].split('.mp4')[0]

        # out_file_name = f'/Users/dingzihan/Documents/projects/tracker_tools/oxe_gripper_anno/bc_z/{v_name}.pkl'
        # out_file_name = f'/Users/dingzihan/Documents/projects/rh20t/joint_angle_demo/cfg_0001/2d_keypoint_anno/{v_name}.pkl'

        text, ok = QInputDialog.getText(self, 'Input Dialog', 'Enter the file path:')
        # 如果点击了确认按钮
        if ok:
            # save_cfg_name = video_name.split('/')[-2]
            # cfg_name_split = save_cfg_name.split('_')
            # cfg_name = f'{cfg_name_split[0]}_{cfg_name_split[1]}'
            # save_root = video_name.split(f'/{cfg_name}_cam')[0]
            v_name = video_name.split('/')[-1]
            save_root = video_name.split(f'/{v_name}')[0]
            print(video_name)
            print(save_root)
            if not os.path.exists(save_root+'/2d_keypoint_anno'):
                os.mkdir(save_root+'/2d_keypoint_anno')
            out_file_name = f'{save_root}/2d_keypoint_anno/{cam_name}_kpt_{text}.pkl'
            pickle.dump(self.anno, open(out_file_name, 'wb'))
    ...
    def get_tap_result(self):        
        ...
        pred_tracks, pred_visibility, images = request_cotracker(self.sam_config, self.co_tracker_config, self.video_pth)
        self.tracker_res = np.array([cv2.cvtColor(image, cv2.COLOR_BGR2RGB) for image in images])
        for k in self.anno.keys():
            video_name = k
        if len(self.anno.keys())>1:
            import pdb; pdb.set_trace()
        self.anno[video_name]['track'] = (pred_tracks, pred_visibility)
